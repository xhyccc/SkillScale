...
ENV_BLOCK='      SKILLSCALE_SKILLS_DIR: /skills
      ...
      SKILLSCALE_WORKERS: "'"${SKILLSCALE_WORKERS}"'"
      SKILLSCALE_HWM: "'"${SKILLSCALE_HWM}"'"
      SKILLSCALE_HEARTBEAT: "'"${SKILLSCALE_HEARTBEAT}"'"
      SKILLSCALE_MATCHER: "'"${SKILLSCALE_MATCHER}"'"
      SKILLSCALE_PYTHON: "'"${SKILLSCALE_PYTHON}"'"
...
services:
  ...
  proxy:
    ...
    environment:
      SKILLSCALE_XSUB_BIND: "${SKILLSCALE_XSUB_BIND}"
      SKILLSCALE_XPUB_BIND: "${SKILLSCALE_XPUB_BIND}"
      SKILLSCALE_METRICS_PORT: "${SKILLSCALE_METRICS_PORT}"
...
cat >> "$COMPOSE_FILE" <<AGENT_SVC
  ...
  agent:
    ...
    environment:
      ...
      SKILLSCALE_SETTLE_TIME: "${SKILLSCALE_SETTLE_TIME}"
    depends_on:
      proxy:
        condition: service_healthy
